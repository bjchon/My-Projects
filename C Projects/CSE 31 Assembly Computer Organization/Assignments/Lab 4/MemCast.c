#include <stdio.h>

int main() {
	int i;
	int four_ints[4];
	char* four_c;

	for(i = 0; i < 4; i++)
        four_ints[i] = 2;

	printf("%x\n", four_ints[0]);
	
    four_c = (char*)four_ints; //this statement casts an int pointer to a char pointer

	for(i = 0; i < 4; i++)
		four_c[i] = 'A' + i; // ASCII value of 'A' is 65 or 0x41 in Hex.

    // Add your code for the exercises here:
	printf("%x\n", four_ints[0]);
	printf("%x\n", four_ints[1]);
	printf("Value in four_ints[0] in hex: %x\n", four_ints[0]);
	printf("Value in four_c: ");
	for(i = 0; i < 4; i++)
		printf("%x", four_c[i]);

	for(i = 0; i < 4; i++)
		printf("\n four_ints hex value: %x, address: %d", four_ints[i], &four_ints[i]);

	for(i = 0; i < 4; i++)
		printf("\n four_c hex value: %x, address: %d", four_c[i], &four_c[i]);
	return 0;
}