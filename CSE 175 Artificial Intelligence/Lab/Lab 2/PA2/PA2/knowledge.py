...
#
# monster_sentences contains the translation of statements into first-order definite clauses
# I received help from our TA Yasemin Gokcen.
#
# Bryant Chon 11/11/22
#
...
monster_sentences = ['Vampire(Celene)', 'Werewolf(Mario)', 'Witch(Mildred)', 
                    ...
                    'Person(x) & Dead(x) ==> Victim(x)', 'Vampire(x) ==> Monster(x)',
                    'Werewolf(x) ==> Monster(x)', 'Witches(x) ==> Monster(x)',
                    ...
                    'Victim(x) & Intact(x) & Complexion(x,Pale) & Boils(y) ==> Poisoned(x)',
