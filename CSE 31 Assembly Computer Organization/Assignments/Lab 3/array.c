...
/* while(a <= b) 
{
    int mid = a + (b-a) / 2; // calculate mid

    if(*(arr+mid) == key) // if array at mid location is equal to key, returns mid
        return mid;

    if(*(arr+mid) > key) // if array at mid location is greater than key b is changed to mid - 1
        b = mid - 1 ;

    else // if array at mid location is less than key a is changed to mid +1
        a = mid + 1;
}  */
    if(b<a)
        return -1;
